...
模型为可替换的 `InitializedResponseModel`，随机初始化一个响应系数，以基线值和治疗后经过时间计算平滑响应。它是未训练的参数化演示模型，不是通用医学模型，也不是训练完成的机器学习预测器。不同药品不会产生经过学习的差异；种子可导致上升或下降，因此不能用于比较药品效果。`trained=false`、`clinically_validated=false` 始终返回；无校准区间，无因果疗效估计。
