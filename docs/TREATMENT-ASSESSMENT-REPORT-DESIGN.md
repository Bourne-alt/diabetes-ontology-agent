...
验收至少覆盖：未来报告不可见；更正/撤回不污染旧快照；真实给药与计划不混；PACS 否定/不确定正确；并用治疗和证据冲突保留；缺少知识时不编结论；伪造引用被拒；真实引文但不支持命题时不得通过；机器学习数值不进入临床结论；LLM 超时后模板报告仍可用；同一报告可回溯至输入/规则/原文/模型版本；原 `/simulate` 和 forecast 回归通过。
