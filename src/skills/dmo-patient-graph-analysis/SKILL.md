...
description: 用糖尿病本体图知识库（dmo 融合查询 API）深度分析患者病情。当用户问某个患者的诊断依据、阈值判定、风险分层、用药安全、"凭什么这么说"、"为什么查不到某个检验/术语"，或需要跨 SQL 事实层与 SPARQL 语义层做证据溯源时使用。本 skill 规定了端点路由决策表、证据链读法与硬性禁令（不输出剂量、不输出概率、不猜术语、不把 Provisional 当确诊）。Use when analyzing a diabetes patient's condition, tracing a clinical conclusion back to guideline provenance, or explaining why a term is unmappable in this repository's ontology + patient fact graph.
license: 与本仓库同许可
compatibility: 需要本仓库的 dmo 服务在 127.0.0.1:8000 可达（uv run dmo serve），或可用 uv run dmo CLI；依赖 PostgreSQL 与 GraphDB(localhost:7200) 已就绪
