...
export default function KnowledgePanel({ run }) {
  ...
  const agentReport = [...run.events].reverse().map(e=>e.raw??e).find(e=>e.type==='assessment_report')?.report;
  useEffect(()=>{if(agentReport)setTab('assessment');},[agentReport]);
  ...
  const graphCard = <div className="graph-card">
    <div className="graph-topline"><strong id="graph-title">{graph.nodes.length ? `${graph.nodes.length} 个节点 · ${graph.edges.length} 条关系` : '等待证据进入图谱'}</strong>
